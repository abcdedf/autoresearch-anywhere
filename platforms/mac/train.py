"""
Autoresearch pretraining script — Mac (MPS/CPU) adaptation.

Based on Karpathy's autoresearch (https://github.com/karpathy/autoresearch)
Mac MPS patches from miolini/autoresearch-macos (https://github.com/miolini/autoresearch-macos)

Key changes from upstream:
- Flash Attention 3 replaced with PyTorch's scaled_dot_product_attention
- Device detection: MPS > CPU (no CUDA assumption)
- torch.compile disabled on MPS (unstable)
- Autocast disabled on MPS (no bf16 autocast support)
- Reduced defaults: DEPTH=4, DEVICE_BATCH_SIZE=16, TOTAL_BATCH_SIZE=2**16
- Window pattern "L" (full context only, simpler for MPS)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # PyTorch SDPA (replaces Flash Attention 3 from upstream)
...
# ---------------------------------------------------------------------------
# Optimizer (MuonAdamW — no torch.compile on MPS)
# ---------------------------------------------------------------------------
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def __init__(self, param_groups):
        ...
        # torch.compile only on CUDA/CPU, not MPS
...
# ---------------------------------------------------------------------------
# Hyperparameters (Mac-tuned defaults)
# ---------------------------------------------------------------------------

ASPECT_RATIO = 64
HEAD_DIM = 128
WINDOW_PATTERN = "L"           # full context only (simpler for MPS)

TOTAL_BATCH_SIZE = 2**16       # ~65K tokens (reduced from upstream 2**19)
EMBEDDING_LR = 0.6
...
DEPTH = 4                     # reduced from upstream 8
DEVICE_BATCH_SIZE = 16        # reduced from upstream 128
...
print(f"{ts()} Device: {device_type}")

# Autocast: only on CUDA/CPU, disabled on MPS
if device_type == "cuda":
...
print(f"{ts()} Vocab size: {vocab_size:,}")
...
print(f"{ts()} Model config: {asdict(config)}")
...
# torch.compile only on CUDA (unstable on MPS)
if device_type == "cuda":
...
print(f"{ts()} Time budget: {TIME_BUDGET}s")
print(f"{ts()} Gradient accumulation steps: {grad_accum_steps}")
print(f"{ts()} Warmup: first 10 steps are not timed (expected ~2-3 min, includes MPS compilation)")
print(f"{ts()} Training starts after warmup, then runs for {TIME_BUDGET}s")
...
while True:
    ...
    if step <= 10:
        phase = f"warmup {step}/10"
    else:
        phase = f"training {int(total_training_time)}/{TIME_BUDGET}s"

    print(f"{ts()} step {step:05d} [{phase}] | loss: {debiased_smooth_loss:.6f} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,}", flush=True)
...
# Final eval
print(f"{ts()} Evaluating val_bpb ({EVAL_TOKENS // 524288} * 524K tokens)...", flush=True)
model.eval()
with autocast_ctx:
    val_bpb = evaluate_bpb(model, tokenizer, DEVICE_BATCH_SIZE)
print(f"{ts()} Evaluation complete.", flush=True)
